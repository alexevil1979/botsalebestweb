...
?>
<div class="container">
    <h1>💬 Диалог #<?= $dialog['id'] ?></h1>

    <div style="background: white; padding: 1.5rem; border-radius: 8px; margin-bottom: 1.5rem;">
        <p><strong>Пользователь:</strong> <?= htmlspecialchars($dialog['first_name'] ?? $dialog['username'] ?? 'N/A') ?></p>
        <p><strong>Telegram ID:</strong> <?= $dialog['telegram_id'] ?></p>
        <p><strong>Текущий шаг:</strong> <?= htmlspecialchars($dialog['current_step'] ?? 'N/A') ?></p>
        <p><strong>Статус:</strong> 
            <span class="status-badge status-<?= $dialog['status'] ?>">
                <?= htmlspecialchars($dialog['status']) ?>
            </span>
        </p>
        <p><strong>Создан:</strong> <?= date('d.m.Y H:i', strtotime($dialog['created_at'])) ?></p>
        <p><strong>Обновлён:</strong> <?= date('d.m.Y H:i', strtotime($dialog['updated_at'])) ?></p>
    </div>
